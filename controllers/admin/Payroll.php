...
class Payroll extends Admin_Controller
{
    ...
    public function manage_salary_details($departments_id = NULL)
    {
        ...
        if (!empty($departments_id)) {
            $department = get_row('tbl_departments', array('departments_id' => $departments_id));
        }
        $companies_id = $this->input->post('companies_id', TRUE);
        if (!empty($companies_id)) {
            $data['companies_id'] = $companies_id;
        } else if (!empty($department->companies_id)) {
            $data['companies_id'] = $department->companies_id;
        } else {
    ...
    public function generate_payslip()
    {
        ...
        if (!empty($flag)) { // check employee id is empty or not
            ...
            if (!empty($data['departments_id'])) {
                $department = get_row('tbl_departments', array('departments_id' => $data['departments_id']));
            }
            $companies_id = $this->input->post('companies_id', TRUE);
            if (!empty($companies_id)) {
                $data['companies_id'] = $companies_id;
    ...
    {
        $data['title'] = lang('provident_fund');
        // active check with current month
        $data['current_month'] = date('m');

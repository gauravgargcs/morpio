...
class Job_Circular extends Admin_Controller
{
    ...
    public function save_job_posted($id = NULL)
    {
        ...
        if (!empty($created) || !empty($edited)) {
            ...
            if(!empty($data['posted_date'])){
                $data['posted_date']=date('Y-m-d H:i', strtotime($data['posted_date']));
            }

            if(!empty($data['last_date'])){
                $data['last_date']=date('Y-m-d H:i', strtotime($data['last_date']));
            }
    ...
    public function delete_jobs_posted($id)
    {
        ...
        $this->job_circular_model->delete_multiple(array('job_circular_id' => $id));
    ...
    function change_application_status($id)
    {
        ...
        if (empty($flag)) {
            ...
            if ($status == 3) {
                ...
                $interview_date = date('Y-m-d H-i', strtotime($this->input->post('interview_date', true)));
                if (!empty($send_email)) {

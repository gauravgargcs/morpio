...
class Settings extends Admin_Controller
{
    ...
    public function save_system()
    {
        $input_data = $this->settings_model->array_from_post(array(
            ...
            'enable_languages', 'allow_client_registration', 'allow_client_project', 'allow_sub_tasks', 'currency_position', 'money_format', 'google_api_key', 'only_allowed_ip_can_clock',
            'auto_close_ticket', 'attendance_report', 'tables_pagination_limit', 'max_file_size', 'recaptcha_secret_key', 'recaptcha_site_key','allowed_files'
        ));
    ...
    public function theme()
    {
        ...
        $can_do = can_do(120);
        if (!empty($can_do)) {
            $data['subview'] = $this->load->view('admin/settings/skote_settings', $data, TRUE);
        } else {
            $data['subview'] = $this->load->view('admin/settings/not_found', $data, TRUE);
        }
        $this->load->view('admin/_layout_skote_main', $data); //page load
    ...
    {
        $input_data = $this->settings_model->array_from_post(array(
            'website_name', 'logo_or_icon', 'sidebar_theme', 'aside-float', 'aside-collapsed', 'layout-boxed', 'layout-fixed', 'login_position', 'RTL',
            ...
            'active_background', 'active_color', 'body_background', 'active_pre_loader','dash_widget_background','dash_widget_text_color','dash_widget_button_color'
        ));
        ...
        if (!empty($_FILES['company_footer_logo']['name'])) {
            $val = $this->settings_model->uploadImage('company_footer_logo');
            $val == TRUE || redirect('admin/settings/theme');
            $input_data['company_footer_logo'] = $val['path'];
        }
        //favicon Process
    ...
    public function translations_industry($lang = null)
    {
        $data['page'] = lang('settings');
        ...
        // }
        // echo "<pre>";
        // var_dump($data['availabe_language']);
        // die;

        $data['industries'] = get_industries();

        $data['load_setting'] = 'translations_industry';
    ...
    public function industry_text_translations()
    {
        $data['page'] = lang('settings');
        $data['industry'] = $industry = $this->input->get('industry');
        ...
        $data['industries'] = get_industries();
        $data['industry_text_translations'] = get_industry_text_translations(['industry_type'=>$industry]);

        $data['load_setting'] = 'industry_text_translations';
        $data['title'] = $industry.' '.lang('translations'); //Page title
        $can_do = super_admin();
        $is_subdomain = is_subdomain();
        if (!empty($can_do && !$is_subdomain)) {
            $data['subview'] = $this->load->view('admin/settings/settings', $data, TRUE);
     ...
     public function add_industry_text_translation($id='')
    {
        $data['page'] = lang('settings');
        $data['industry'] = $industry = $this->input->get('industry');
        $data['lang'] = $lang = $this->input->get('lang');
        $data['id'] = $id;
        $data['word'] = '';
        $data['translation'] = '';
        if($_POST){
       ...
       $this->form_validation->set_rules('industry_type',"Industry", 'required');
       ...
       if ($this->form_validation->run() == FALSE ) {
        ...
        $insert_data['word'] = $this->input->post('word');
            $insert_data['translation'] =  $this->input->post('translation');
            $insert_data['language'] =  $this->input->post('language');
            $insert_data['industry_type'] =  $this->input->post('industry_type');
        if($id){

            $this->db->where('id',$id)->update('tbl_industry_lang',$insert_data);
        }else{
            $this->db->insert('tbl_industry_lang',$insert_data);

        }
        $type = 'success';

        $message = lang('language_update_successfully');

        set_message($type, $message);
        redirect('admin/settings/industry_text_translations?industry='. $insert_data['industry_type']);
    }
}
        ...
        $data['industries'] = get_industries();
         ...
         if($id){
            ...
            $old_data= get_industry_text_translations(['id'=>$id]);
            $data['word'] = $old_data[0]['word'];
            $data['translation'] = $old_data[0]['translation'];
            $data['lang'] = $old_data[0]['language'];
            $data['industry'] = $old_data[0]['industry_type'];
         }
         // var_dump($data['active_language']);die();
        $data['load_setting'] = 'add_industry_text_translation';
        $data['title'] = $industry.' '.lang('translations'); //Page title
        $can_do = super_admin();
        $is_subdomain = is_subdomain();
        if (!empty($can_do && !$is_subdomain)) {
    ...
    public function delete_industry_text_translation($id)
    {

            $this->settings_model->_table_name = 'tbl_industry_lang';
            $this->settings_model->_primary_key = 'id';
            $this->settings_model->delete($id);
            // messages for user
            $type = "success";
            $message = lang('Industry language word') . ' ' . lang('deleted');
            set_message($type, $message);
    ...
    public function lead_status($action = NULL, $id = NULL)
    {
        ...
        if ($action == 'update_lead_status') {
            if (!empty($created) || !empty($edited)) {
                ...
                if (!empty($cate_data['lead_status'])) {
                    // update root category
                    ...
                    $type = "error";
                    $msg = lang('please_select_lead_status');
                }
                $message = $msg;
                set_message($type, $message);
            }
    ...
    public function save_tickets()
    {
        ...
        $input_data = $this->settings_model->array_from_post(array(
            'default_department', 'default_status', 'default_priority',
    ...
    public function database_backup()
    {
        ...
        $data['backups']= get_result('tbl_db_backup');
        $can_do = can_do(136);
        if (!empty($can_do)) {
            $data['subview'] = $this->load->view('admin/settings/settings', $data, TRUE);
        } else {
    ...
    public function db_backup()
    {
        $this->load->helper('file');
        $this->load->dbutil();
        $prefs = array('format' => 'zip', 'filename' => 'BD-backup_' . date('Y-m-d') . '_' . date('H-i'));
                $backup = $this->dbutil->backup($prefs);
       $file_name = 'DB-manual-backup_'.$this->db->database .'-'. date('Y-m-d_H-i') . '.zip';
    ...
    function download_backup($id)
    {
        ...
        $data['backups']= get_result('tbl_db_backup',['id'=>$id]);
        $backups =   $data['backups'][0];
        // var_dump($backups);die;
        if( $backups){
            $this->load->helper('file');
            $this->load->helper('download');

            $data = file_get_contents('./uploads/backup/' . $backups->file);
            // var_dump($data);die;
            force_download($backups->file, $data);
         }
        // redirect('admin/settings/database_backup');
    }
    ...
    public function delete_backup($id)
    {
        ...
        $this->settings_model->_table_name = 'tbl_db_backup';
        $this->settings_model->_primary_key = 'id';
        $this->settings_model->delete($id);
    ...
    public function all_currency($code = null, $del = null)
    {
        $data['page'] = lang('settings');
        $data['load_setting'] = 'all_currency';
        $data['title'] = lang('all_currency'); //Page title
        if (!empty($del)) {
            $this->settings_model->_table_name = "tbl_currencies"; // table name
            $this->settings_model->delete_multiple(array('code' => $code));
            $type = "success";
            $message = 'Currency Information successfully deleted';
            set_message($type, $message);
            redirect('admin/settings/all_currency');
        }
        if (!empty($code)) {

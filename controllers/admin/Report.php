...
class Report extends Admin_Controller
{
    ...
    public function account_statement()
    {
        ...
        if (!empty($data['account_id'])) {
            $data['report'] = TRUE;
            $data['start_date'] = date('Y-m-d H:i', strtotime($this->input->post('start_date', TRUE)));
            $data['end_date'] = date('Y-m-d H:i', strtotime($this->input->post('end_date', TRUE)));
    ...
    function get_account_statement($account_id, $start_date, $end_date, $transaction_type)
    {
        if ($transaction_type == 'all_transactions') {
            $where = array('account_id' => $account_id, 'date >=' => date('Y-m-d H:i', strtotime($start_date)), 'date <=' => date('Y-m-d H:i', strtotime($end_date)));
        } elseif ($transaction_type == 'debit') {
            $where = array('account_id' => $account_id, 'date >=' => date('Y-m-d H:i', strtotime($start_date)), 'date <=' => date('Y-m-d H:i', strtotime($end_date)), 'credit' => $transaction_type);
        } else {
            $where = array('account_id' => $account_id, 'date >=' => date('Y-m-d H:i', strtotime($start_date)), 'date <=' => date('Y-m-d H:i', strtotime($end_date)), 'debit' => $transaction_type);
        }
    ...
    public function account_statement_pdf($account_id, $start_date, $end_date, $transaction_type)
    {
        ...
        $data['all_transaction_info'] = $this->get_account_statement($account_id, date('Y-m-d H:i', strtotime($start_date)), date('Y-m-d H:i', strtotime($end_date)), $transaction_type);
        $data['title'] = lang('account_statement');
        $this->load->helper('dompdf');
        $viewfile = $this->load->view('admin/report/account_statement_pdf', $data, TRUE);
        pdf_create($viewfile, lang('account_statement') . ' From:' . date('Y-m-d H:i', strtotime($start_date)) . ' To:', date('Y-m-d H:i', strtotime($end_date)));
    }
    ...
    public function tasks_assignment()
    {
        ...
        $permission_user = $this->report_model->allowad_user('57');
        // get all admin user
    ...
    public function bugs_assignment()
    {
        ...
        $data['all_project'] = $this->report_model->get_permission('tbl_project');
        // get permission user by menu id
        $permission_user = $this->report_model->allowad_user('58');
        // get all admin user
        $admin_user = get_result('tbl_users', array('role_id' => 1));
        // if not exist data show empty array.
        if (!empty($permission_user)) {
    ...
    public function tasks_report()
    {
        $data['title'] = lang('project_report');
        // get permission user by menu id
        $permission_user = $this->report_model->allowad_user('54');
        // get all admin user
        $admin_user = get_result('tbl_users', array('role_id' => 1));
        // if not exist data show empty array.
        if (!empty($permission_user)) {
    ...
    public function tickets_report()
    {
        ...
        // get permission user by menu id
        $permission_user = $this->report_model->allowad_user('7');
        // get all admin user
        $admin_user = get_result('tbl_users', array('role_id' => 1));
        // if not exist data show empty array.
        if (!empty($permission_user)) {

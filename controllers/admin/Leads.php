...
class Leads extends Admin_Controller
{
    ...
    public function index($id = NULL)
    {
        ...
        if (!empty($id)) {
            if ($id == 'kanban') {
                $data['active'] = 1;
                $_SESSION['leads_kanban'] = $id;
                
            } elseif ($id == 'list') {
                $data['active'] = 1;
                unset($_SESSION['leads_kanban']);
               } elseif ($id == 'new') {
                $data['active'] = 2;
                 unset($_SESSION['leads_kanban']);
     ...
     public function list($id = NULL)
    {
        ...
        // $data['subview'] = $this->load->view('admin/leads/all_leads2', $data, TRUE);
        $this->load->view('admin/_layout_skote_main', $data); //page load
        // $this->output->enable_profiler(TRUE);
    ...
    function saved_leads($id = NULL)
    {
        ...
        // update root category
        $where = array('client_id' => $data['client_id'], 'contact_name' => $data['contact_name']);
        // duplicate value check in DB
        if (!empty($id)) { // if id exist in db update data
        ...
        if (!empty($check_leads)) { // if input data already exist show error alert
            // massage for user
            $type = 'error';
            $msg = "<strong style='color:#000'>" . $data['contact_name'] . '</strong>  ' . lang('already_exist');
        } else { // save and update query
                ...
                foreach ($notifiedUsers as $users) {
                    if ($users != $this->session->userdata('user_id')) {
                        add_notification(array(
                            ...
                            'value' => lang('lead') . ' ' . $leads_info->contact_name,
                        ));
                    }
                }
    ...
    function converted($leads_id)
    {
        ...
        $data['permission'] = $leads_info->permission;
        $data['companies_id'] = $leads_info->companies_id;
            ...
            foreach ($notifiedUsers as $users) {
                if ($users != $this->session->userdata('user_id')) {
                    add_notification(array(
                        ...
                        'link' => 'admin/leads/leads_details/' . $leads_info->leads_id,
                        'value' => lang('lead') . ' ' . $leads_info->contact_name . ' ' . lang('client') . ' ' . $client_info->name,
                    ));
                }
            }
            show_notification($notifiedUsers);
    ...
    function update_member($id)
    {
        ...
        if (!empty($can_edit)) {
            ...
            $activities = array(
                ...
                'icon' => 'fa-rocket',
                'link' => 'admin/leads/leads_details/' . $id,
                'value1' => $leads_info->contact_name,
            );
    ...
    public function multiple_lead_update_users()
    {
        // get permission user by menu id
        $data['assign_user'] = $this->items_model->allowad_user('55');

        $data['modal_subview'] = $this->load->view('admin/leads/_multiple_lead_modal_users', $data, FALSE);
        $this->load->view('admin/_layout_skote_modal', $data);
    }

    public function mutiple_lead_update_member()
    {
        $leads_ids= $this->input->post('leads_id');
        if($leads_ids){
            foreach ($leads_ids as $key => $id) {
                $can_edit = $this->items_model->can_action('tbl_leads', 'edit', array('leads_id' => $id));
                if (!empty($can_edit)) {
                    $leads_info = $this->items_model->check_by(array('leads_id' => $id), 'tbl_leads');
                    ...
                    if (!empty($permission)) {
                        if ($permission == 'everyone') {
                            $assigned = 'all';
                            $assigned_to['assigned_to'] = $this->items_model->allowad_user_id('55');
                        } else {
                            $assigned_to = $this->items_model->array_from_post(array('assigned_to'));
                            if (!empty($assigned_to['assigned_to'])) {
                                foreach ($assigned_to['assigned_to'] as $assign_user) {
    ...
    public function multiple_lead_change_status(){
        $lead_status_id = $this->input->post('status') ;
        $leads_ids = $this->input->post('leads_id') ;
        
        if($leads_ids){
            foreach ($leads_ids as $key => $leads_id) {

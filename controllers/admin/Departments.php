...
class Departments extends Admin_Controller
{
    ...
    public function save_departments($id = NULL)
    {
        ...
        if (!empty($created) || !empty($edited)) {
            ...
            if (!empty($check_department)) {
                ...
                $emailRes=$this->createDeptEmailAccount($data,$departments_id);
                if(!empty($emailRes)){
                    $type = "error";
                    $message=$emailRes;
                    set_message($type, $message);
                ...
                foreach ($menu['menu'] as $key => $v_menu) {
                    if ($v_menu != 'undefined') {
                        if ($v_menu != $d_menu || $v_menu != $c_menu) {
                            ...
                            $mdata['view'] = ($view ? menu_by_company_c_id($view,$companies_id) : 0);
                            $mdata['created'] = ($created ? menu_by_company_c_id($created,$companies_id) : 0);
                            $mdata['edited'] = ($edited ? menu_by_company_c_id($edited,$companies_id) : 0);
                            $mdata['deleted'] = ($deleted ? menu_by_company_c_id($deleted,$companies_id) : 0);
                            $mdata['menu_id'] = menu_by_company_c_id($v_menu,$companies_id);
                            $mdata['designations_id'] = $desi_id;
    ...
    public function get_comany_menu($id = null)
    {
        $data['companies_id'] = $id;
        $_data['all_menu'] = $this->load->view('admin/department/all_menu', $data, true);
    ...
    public function edit_departments($id)
    {
        ...
        if (!empty($created) || !empty($edited)) {
            ...
            if (!empty($post['deptname'])) {
                ...
                $emailRes= $this->createDeptEmailAccount($post,$id);
                if(!empty($emailRes)){
                    $type = "error";
                    $message=$emailRes;
                    set_message($type, $message);
    ...
    function createDeptEmailAccount($post=array(),$departments_id=0){
        $sub='';
        ...
        $deptnameSlug = strtolower(preg_replace('/\s+/','',$deptname));
       
        if(!empty($sub)){
            $email_user= $deptnameSlug.'.'.$sub;
        }else{
            $email_user= $deptnameSlug;
        }
        
        $this->load->helper('string');
    
        $email_pass = random_string('alnum', 15); // email password
        $email_quota = 1024; // 0 is no quota, or set a number in mb
       
        //XMLAPI cpanel client class
        require APPPATH . "third_party/Cpanel_Api.php";
        $xmlapi = new Cpanel_Api();
      
        $cpuser = $xmlapi->get_user();
        $email_domain = $xmlapi->get_host();
        
        $call = array('domain'=>$email_domain, 'email'=>$email_user, 'password'=>$email_pass, 'quota'=>$email_quota);
        $res = $xmlapi->api2_query($cpuser, "Email", "addpop", $call );

        $result= json_decode($res);
        if(isset($result->cpanelresult->error)){
            $data= $result->cpanelresult->error; //show the result of your query
        }else{
            $data=NULL;
            $dept_arr=array('cp_email_domain'=>$email_domain, 'cp_email_user'=>$email_user, 'cp_email_password'=>encrypt($email_pass), 'cp_email_quota'=>$email_quota);
            $this->department_model->_table_name = "tbl_departments"; // table name
            $this->department_model->_primary_key = "departments_id"; // $id
            $this->department_model->save($dept_arr, $departments_id);
        }    

...
class Invoice extends Admin_Controller
{
    ...
    function recurring_invoice($id = NULL)
    {
        ...
        $data['all_client'] = $this->invoice_model->get();
        // get permission user
        $data['permission_user'] = $this->invoice_model->allowad_user('51');
        // get all invoice
        $data['all_invoices_info'] = $this->invoice_model->get_permission('tbl_invoices');
    ...
    function cloned_invoice($id)
    {
        ...
        if (!empty($can_edit) && !empty($edited)) {
            ...
            $new_invoice = array(
                ...
                'invoice_date' => date('Y-m-d H:i', strtotime($this->input->post('invoice_date', true))),
        ...
        if (!empty($can_edit) && !empty($edited)) {
            ...
            if ($paid_amount != 0) {
                if ($paid_amount > $due) {
                    ...
                    if ($inv_info->status == 'draft') {
                        $qty_calculation = config_item('qty_calculation_from_items');
                        $invoice_items = $this->invoice_model->ordered_items_by_id($invoices_id);
                        if (!empty($invoice_items)) {
    ...
    function change_invoice_status($action, $id)
    {
        if ($action == 'mark_as_sent') {
            $inv_info = $this->invoice_model->check_by(array('invoices_id' => $id), 'tbl_invoices');
            if ($inv_info->status == 'draft') {
                $qty_calculation = config_item('qty_calculation_from_items');
                $invoice_items = $this->invoice_model->ordered_items_by_id($id);
                if (!empty($invoice_items)) {
    ...
    function tax_rates($action = NULL, $id = NULL)
    {
        ...
        if ($action == 'delete_tax_rates') {
            ...
            $subview = 'tax_rates';
            // get permission user
            $data['permission_user'] = $this->invoice_model->allowad_user('16');
            // get all invoice
    ...
    public function reminder($module, $module_id, $id = null)
    {
        ...
        if ($this->input->post()) {
            ...
            $r_data['date'] = date('Y-m-d H:i', strtotime($this->input->post('date', true)));

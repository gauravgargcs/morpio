...
class Dashboard extends Admin_Controller
{
    ...
    public function index($action = NULL)
    {
        ...
        //Added By Jaraware infosoft - 05 dec 21
        $data['proposal_history'] = $this->admin_model->getProposalHistory(); 
    ...
    public function set_clocking($id = NULL, $user_id = null, $row = null, $redirect = null)
    {
        if (!empty(attendance_access())) {
            ...
            $date = date('Y-m-d H-i', strtotime($this->input->post('clock_date', TRUE)));
    ...
    function mark_attendance()
    {
        ...
        $date = date('Y-m-d H-i', strtotime($this->input->post('attend_date', TRUE)));
    ...
    {
        $data = $this->admin_model->array_from_post(array('user_id', 'title','notes', 'status'));
        $data['due_date']=date('Y-m-d H-i',strtotime($this->input->post('due_date')));
    ...
    public function multiple_todo_change_status()
    {

        $_status = $this->input->post('status', true);
        if (!empty($_status)) {
            $todo_ids = $this->input->post('todo_id', true);        
            if($todo_ids){
                foreach ($todo_ids as $key => $todo_id) {
                    ...
                    $data_return['message'] =  lang('todo_status_change');
                }
            }else{
    ...
    public function all_todo($id = null)
    {
        ...
        if ($id == 'kanban') {
            ...
            if ($id>0) {
                $data['active'] = $id ;
                $filterByStatus = array('status' => $id);
            }else{
                $data['active'] = 0;
            }
        }

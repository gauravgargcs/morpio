...
class Leave_Management extends Admin_Controller
{
    ...
    public function save_leave_application()
    {
        ...
        if (!empty($data['leave_type'])) {
            if ($data['leave_type'] == 'single_day') {
                $start_date = date('Y-m-d H:i', strtotime($this->input->post('single_day_start_date', true)));
                $end_date = null;
                $hours = null;
            }
            if ($data['leave_type'] == 'multiple_days') {
                $start_date = date('Y-m-d H:i', strtotime($this->input->post('multiple_days_start_date', true)));
                $end_date = date('Y-m-d H:i', strtotime($this->input->post('multiple_days_end_date', true)));
                $hours = null;
            }
            if ($data['leave_type'] == 'hours') {
                $start_date = date('Y-m-d H:i', strtotime($this->input->post('hours_start_date', true)));
                $end_date = null;
    ...
    function check_available_leave($user_id, $start_date = NULL, $end_date = NULL, $leave_category_id = NULL)
    {
        ...
        if (!empty($leave_category_id) && !empty($start_date)) {
            ...
            // $total_taken = $total_taken . ' days ' .  number_format($total_hourly * $office_hours, 2) . 'hours';
    ...
    //                 $t = $profile->fullname;
    //             }
    //             echo "$t already took  $total_taken $total_leave->leave_category You can apply maximum for $left_leave more";
    //             exit();
    //         }
    //     } else {
    //         return lang('all_required_fill');

...
class User extends Admin_Controller
{
    ...
    public function user_list($action = NULL, $id = NULL)
    {
        ...
        $data['permission_user'] = $this->user_model->allowad_user('24');
    ...
    public function user_details($id, $active = null)
    {
        ...
        $date = date('Y-m-d H:i', strtotime($this->input->post('date', true)));
        if (!empty($date)) {
            $data['date'] = $date;
        } else {
            $data['date'] = date('Y-m');
    ...
    public function update_details($id)
    {
        ...
        if(!empty($data['joining_date'])){
            $data['joining_date']=date('Y-m-d H:i', strtotime($data['joining_date']));
        }
        if(!empty($data['date_of_birth'])){
            $data['date_of_birth']=date('Y-m-d H:i', strtotime($data['date_of_birth']));
        }
    ...
    public function save_user()
    {
        ...
        if (!empty($created) || !empty($edited)) {
            $login_data = $this->user_model->array_from_post(array('username', 'email', 'role_id','companies_id'));
    ...
    function send_email_reset_password($email, $user_info, $password)
    {
        ...
        public function connect_listing($id)
    {
        $user_info = $this->db->where('user_id', $id)->get('tbl_users')->row();
        $this->db->where('is_listing_connected',1)->update('tbl_users',['is_listing_connected'=>0]);
           if($user_info){
                  $this->user_model->_table_name = 'tbl_users'; // table name
                $this->user_model->_primary_key = 'user_id'; // $id
                $this->user_model->save(['is_listing_connected'=>1], $id); 
           }

        $type = 'success';
        $message = lang('Account connected successfully');
        set_message($type, $message);
        redirect('admin/user/user_list'); //redirect page
    }
}

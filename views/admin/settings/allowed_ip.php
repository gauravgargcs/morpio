...
<div class="card">
    ...
    <div class="card-body">
        <h4 class="card-title mb-4"><?= lang('allowed_ip') ?></h4>
                        ...
                        <tr id="allowed_ip_<?= $allowed_ip->allowed_ip_id ?>">
                            ...
                            <td>
                                ...
                                <?php } else { ?>
                                    ...
                                    <?php $status = 'danger';
                                    $action = '<a class="btn btn-sm btn-success" data-bs-toggle="tooltip" title="' . lang('click_to') . ' ' . lang('active') . '" href="' . base_url("admin/settings/allowed_ip/change_status/active/$allowed_ip->allowed_ip_id") . '"><i class="fa fa-check"></i></a>';
                                    if ($allowed_ip->status == 'active') {
                                        $status = 'success';
                                        $action = '<a class="btn btn-sm btn-danger" data-bs-toggle="tooltip" title="' . lang('click_to') . ' ' . lang('reject') . '" href="' . base_url("admin/settings/allowed_ip/change_status/reject/$allowed_ip->allowed_ip_id") . '"><i class="fa fa-times"></i></a>';

                                    } else if ($allowed_ip->status == 'pending') {
                                        $status = 'warning';
                                        $action = '<a class="btn btn-sm btn-success" data-bs-toggle="tooltip" title="' . lang('click_to') . ' ' . lang('active') . '" href="' . base_url("admin/settings/allowed_ip/change_status/active/$allowed_ip->allowed_ip_id") . '"><i class="fa fa-check"></i></a>' . ' ' . '<a class="btn btn-xs btn-danger" data-bs-toggle="tooltip" title="' . lang('click_to') . ' ' . lang('reject') . '" href="' . base_url("admin/settings/allowed_ip/change_status/reject") . '"><i class="fa fa-times"></i></a>';
                                    }
                                    ?>
                                    <strong class="badge badge-soft-<?= $status ?>"><?= lang($allowed_ip->status); ?></strong>
                                <?php } ?>
                            </td>
                            <?php if (!empty($edited) || !empty($deleted)) { ?>
                                <td>
                                    ...
                                    <?php } else { ?>
                                        ...
                                        <?php if (!empty($deleted)) { ?>
                                            <?php echo ajax_anchor(base_url("admin/settings/allowed_ip/delete_allowed_ip/" . $allowed_ip->allowed_ip_id), "<i class='btn btn-outline-danger btn-sm fa fa-trash-o' style='height:26px;'></i>", array("class" => "", "title" => lang('delete'), "data-fade-out-child-on-success" => ".child", "data-fade-out-on-success" => "#allowed_ip_" . $allowed_ip->allowed_ip_id)); ?>
                                        <?php }
                                    }
                                    ?>
                                </td>

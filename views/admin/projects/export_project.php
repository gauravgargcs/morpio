    ...
    <tr>
        <td style="width: 40%;float: right">
            ...
            <div class="name"><strong><?= lang('created_date') ?>
                    :</strong><?= display_datetime($project_details->created_time) ?>
            </div>
            <div class="name"><strong><?= lang('start_date') ?>
                    :</strong><?= display_datetime($project_details->start_date) ?></div>
...
<table class="items" width="100%">
    ...
    foreach ($permission_users as $v_users) {
        ...
        ?>
        <tr>
            <td><?= fullname($v_users->user_id) ?></td>
        ...
        if (!empty($all_milestones_info)) {
            foreach ($all_milestones_info as $key => $v_milestones) {
                ...
                <tr>
                    ...
                    <td style="text-align: center"><?= display_datetime($v_milestones->start_date) ?></td>
                    <?php
                    $due_date = $v_milestones->end_date;
                    $due_time = strtotime($due_date);
                    $current_time = time();
                    ?>
                    <td style="text-align: center"><?php echo display_datetime($due_date);
                        if ($current_time > $due_time && $progress < 100) {
                            echo "<span style='color:red'>" . lang('overdue') . "</span>";
                        }
                        ?></td>
                    <td style="text-align: center"><?= fullname($v_milestones->user_id) ?></td>
        ...
        if (!empty($all_task_info)) {
            foreach ($all_task_info as $v_u_tasks) {
                ...
                <tr>
                    ...
                    <td style="text-align: center"><?= display_datetime($v_u_tasks->task_start_date) ?></td>
                    <td style="text-align: center"><?= display_datetime($v_u_tasks->due_date) ?></td>
        ...
        if (!empty($total_timer)) {
            foreach ($total_timer as $v_timer) {
                ...
                if (!empty($aproject_info)) {
                    ?>
                    <tr>
                        <td><?= fullname($v_timer->user_id); ?></td>
                ...
                if (!empty($total_tasks_timer)) {
                    foreach ($total_tasks_timer as $v_task_timer) {
                        ...
                        if (!empty($task_info)) {
                            ?>
                            <tr>
                                <td><?= fullname($v_task_timer->user_id); ?></td>
                                <td><?= $v_u_tasks->task_name ?></td>
        ...
        if (!empty($all_tickets_info)) {
            foreach ($all_tickets_info as $v_tickets_info) {
                ...
                <tr>
                    ...
                    <td style="text-align: center"><?= display_datetime($v_tickets_info->created) ?></td>
                    <?php if ($this->session->userdata('user_type') == '1') { ?>
                        <td><?= fullname($v_tickets_info->reporter); ?></td>
                    <?php } ?>

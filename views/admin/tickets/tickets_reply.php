...
?>
<div class="d-flex py-3 border-top" id="ticket-reply-container-<?php echo $v_replies->tickets_replies_id ?>">
    ...
    <div class="flex-grow-1">
        <h5 class="font-size-14 mb-1">
            <a href="<?= base_url() ?>admin/user/user_details/<?= $v_replies->replierid ?>"> <?= ($profile_info->fullname) . ' ' . $label ?></a> 
            <small class="text-muted float-end">
                <?= time_ago($v_replies->time) ?>
        ...
        <p class="text-muted"><?php if (!empty($v_replies->body)) echo $v_replies->body; ?></p>
        <div>
            <?php
            ...
            <a href="<?= base_url() ?>admin/projects/download_files/<?= $v_files->fileName . '/' . true ?>">
                ...
                <?php } else { ?>
                    <div class="mail-attachment-info">
                        <a href="<?= base_url() ?>admin/tickets/index/download_file/<?= $v_replies->tickets_id . '/' . $v_files->fileName ?>" class="mail-attachment-name"><i class="fa fa-paperclip"></i> <?= $v_files->size ?> <?= lang('kb') ?>
                        </a>

                        <a href="<?= base_url() ?>admin/tickets/index/download_file/<?= $v_replies->tickets_id . '/' . $v_files->fileName ?>" class="btn btn-default btn-xs pull-right"><i class="fa fa-cloud-download"></i></a>
            ...
            ?>
            <button class="text-primary reply" data-bs-toggle="tooltip" data-bs-placement="top" title="<?= lang('click_to_reply') ?>" id="reply__<?php echo $v_replies->tickets_replies_id ?>"><i class="fa fa-reply "></i> <?= lang('reply') ?>
            </button>
        </div>
        ...
        <div class="reply__" id="reply_<?php echo $v_replies->tickets_replies_id ?>">
            <form id="<?php echo $comment_reply_type; ?>-comment-form-<?php echo $v_replies->tickets_replies_id ?>" action="<?php echo base_url() ?>admin/tickets/save_comments_reply/<?php   if (!empty($v_replies->tickets_replies_id)) { echo $v_replies->tickets_replies_id; } ?>" method="post">

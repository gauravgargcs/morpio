...
<form role="form" id="from_items" action="<?php echo base_url(); ?>admin/estimates/add_insert_items/<?= $estimates_id ?>" method="post" class="form-horizontal">
            ...
            <table class="table table-striped">
                ...
                <tr>
                    <th>
                        <div class="form-check form-check-primary mb-3 mt-sm">
                            ...
                            <label class="form-check-label" data-bs-toggle="tooltip" title="<?= lang('select') . ' ' . lang('all') ?>"></label>
                        </div>
                    </th>
                    <th><?= lang('item') ?></th>
                    <?php
                ...
                if (!empty($saved_items)) {
                    ...
                    foreach ($saved_items as $group_id => $v_saved_items) {
                        ...
                        ?>
                        <tr>
                        <th colspan="6" style="font-size: 16px"><?= $group ?></th>
                        <?php
                        if (!empty($v_saved_items)) {
                            foreach ($v_saved_items as $v_item) { ?>
                                <tr>
                                    <td>
                                        <div class="form-check form-check-primary mb-3 mt-sm">
                                            <input class="child_present form-check-input" type="checkbox" name="saved_items_id[]" value="<?= $v_item->saved_items_id ?>"/>

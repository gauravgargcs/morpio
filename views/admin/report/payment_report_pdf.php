...
<div style="padding:35px 0 50px;text-align:center"><span
    ...
    <?php
    if (!empty($range[0])) {
        $start_date = display_datetime($range[0]);
        $end_date = display_datetime($range[1]);
    }
    if (!empty($start_date)) { ?>
        ...
        foreach ($all_payments as $key => $payments) {
            ...
            <tr style="border-bottom:1px solid #ededed">
                <td style="padding: 10px 0px 10px 10px;"
                    valign="top"><?= display_datetime($payments->payment_date) ?></td>
                <td style="padding: 10px 10px 5px 10px;word-wrap: break-word;"
                    valign="top">
                    <?= display_datetime($invoice->invoice_date) ?>
                </td>
                <td style="padding: 10px 10px 5px 10px;word-wrap: break-word;"

...
//        add_action('before_add_online_payment_modes', [ $this, 'initMode' ]);
    ...
    public function process_payment($data)
    {
        ...
        $gateway->setApiKey(getConfigItems('mollie_api_key'));

        $oResponse = $gateway->purchase([
            'amount' => number_format($data['total'], 2, '.', ''),
            'description' => getDescription($data, true),
            'returnUrl' => site_url('payment/mollie/verify_payment'),
            'notifyUrl' => site_url('payment/mollie/webhook'),
            'metadata' => [
                'order_id' => $data['subscriptions_id'],
            ],
        ])->send();
        ...
        if ($oResponse->isRedirect()) {
            ...
            $message = 'Error ' . $oResponse->getCode() . ': ' . $oResponse->getMessage();
            set_message('error', $message);
        }

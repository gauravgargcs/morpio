...
//        add_action('before_render_payment_gateway_settings', 'two_checkout_ssl_notice');
    ...
    public function process_payment($data)
    {
        ...
        redirect(site_url('payment/checkout/make_subs_payment'));

    }
    public function finish_payment($data)
    {
        if (getConfigItems('two_checkout_live') == 'TRUE') {
            $mode = 'FALSE';
        } else {
            $mode = 'TRUE';
        }
        $gateway = Omnipay::create('TwoCheckoutPlus_Token');
        $gateway->setAccountNumber(getConfigItems('2checkout_seller_id'));
        $gateway->setPrivateKey(getConfigItems('2checkout_private_key'));
        ...
        $oResponse = $gateway->purchase([
            'amount' => number_format($data['input_data']->total, 2, '.', ''),
            'currency' => $data['input_data']->currency,
            'token' => $this->ci->input->post('token'),
            'transactionId' => $data['input_data']->subscriptions_id,
            'card' => $billing_data,
        ])->send();
        return $oResponse;
    }

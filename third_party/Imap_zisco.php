...
class Imap_zisco
{
    ...
    public function __construct($mailbox, $username, $password, $encryption = false)
    {
        ...
        if ($encryption != null && isset($encryption) && $encryption == 'ssl') {
            $enc = ':993/imap/ssl/novalidate-cert';
        } elseif ($encryption != null && isset($encryption) && $encryption == 'tls') {
            $enc = ':993/imap/tls/novalidate-cert';
        }
        $this->mailbox = '{' . $mailbox . $enc . '}INBOX';
    ...
    function my_imap_status($stream, $mailbox='', $info=SA_ALL)
    {
        ...
        if( (empty($mailbox)) || ( $mailbox == $curr_obj->Mailbox) ) {  
            return $curr_obj;
        }
        ...
        $curr_obj=imap_check($stream);
        //    var_dump($current_mailbox);die;
        return $curr_obj;
    }

    /**
     * returns newest emails on top
     *
     * @return array messages
     * @param $search filter search emails
     */
    public function getEmails($search = 'ALL')
    {
        $emails = [];
  ...
  /**
     ...
     * @param $withbody without body
     */
    public function getNewMessages($withbody = true, $folder)
    {
        $emails = [];
        $date = date ( "d M Y", strToTime ( "-2 days" ) );
    // $uids = imap_search ( $mbox, "SINCE \"$date\"", SE_UID );
        $result = imap_search($this->imap, "SINCE \"$date\"" );
    ...
    protected function formatMessage($id, $withbody = true)
    {
        ...
        $email = [
            ...
            'from' => $this->toAddress($header->from[0],'array'),
            'date' => $header->date,
            'subject' => $subject,
            'uid' => $uid,
     ...
     */
    protected function toAddress($headerinfos,$format='string')
    {
        $email = '';
        $name = '';
        $charset = '';

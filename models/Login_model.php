...
class Login_Model extends MY_Model
{
        ...
        if (!empty($admin)) {
            ...
            if ($admin->role_id != '2') {
                ...
                $mark_attendance = $this->input->post('mark_attendance', true);
                    ...
                    $ssoData = $this->setSSOToken($admin);
                    ...
                    $data = array(
                        ...
                        'user_token' => $ssoData['user_token'],
                        'identity_token' => $ssoData['identity_token'],
    ...
    * @return void
    */
    public function setSSOToken($admin) {
        log_message('error', $admin->email);
        $user_token = $admin->user_token;
        $identity_token = $admin->identity_token;
        $user_id = $admin->user_id;
        if(empty($user_token)){
            $check_data_json = array(
                'request' => array(
                    'user' => array(
                        'login' => $admin->email
                    )
            ...
            log_message('error', $admin->email.'1');
            $check_data_json = json_encode($check_data_json,TRUE);
            log_message('error', $admin->email.'2');
            $check_data_res = check_user($check_data_json, $admin->user_id);
            log_message('error', $admin->email.'3');
            if (isset($check_data_res['user_token']) && !empty($check_data_res['user_token'])) {
                $user_token = $check_data_res['user_token'];
                $identity_token = $check_data_res['identity_token'];
                $userda = array('user_token' => $user_token,'identity_token' => $identity_token);
                // update user token & identity token
                update_user($userda, $admin->user_id);
            } else {
                $insert_data_json = array(
                    'request' => array(
                        'user' => array(
                            ...
                            'identity' => array(
                                'name' => array(
                                    'givenName' => $admin->username,
                                    'familyName' => $admin->username,
                                    'formatted' => $admin->username,
                ...
                $created_data = create_user($insert_data_json, $user_id);
        ...
        log_message('error', json_encode($ssoData));
        return $ssoData;
    }

...
class Purchase_Model extends MY_Model
{
    ...
    public function get_payment_status($purchase_id)
    {
        if (!empty($purchase_id)) {
            ...
            if ($purchase_info->status == 'draft') {
                return ('draft');
            } elseif ($purchase_info->status == 'Cancelled') {
                return ('cancelled');
    ...
    public function get_purchase_paid_amount($purchase_id)
    {
        ...
        if (!empty($companies_id)) {
            if ($this->db->field_exists('companies_id', 'tbl_purchase_payments')) {

...
class Performance_Model extends MY_Model
{
    ...
    public function get_all_indicator_info($performance_indicator_id = NULL)
    {
        ...
        if (!empty($performance_indicator_id)) {
            ...
            $companies_id = $this->session->userdata('companies_id');
            if (!empty($companies_id)) {
                if ($this->db->field_exists('companies_id', 'tbl_performance_indicator')) {
                    $this->db->where('tbl_performance_indicator.companies_id', $companies_id);
                }
            }
            $query_result = $this->db->get();
            $result = $query_result->result();
    ...
    public function get_appraisal_value_by_id($id = NULL)
    {
        ...
        if (!empty($id)) {
            ...
            $companies_id = $this->session->userdata('companies_id');
            if (!empty($companies_id)) {
                if ($this->db->field_exists('companies_id', 'tbl_performance_apprisal')) {
                    $this->db->where('tbl_performance_apprisal.companies_id', $companies_id);
                }
            }
            $query_result = $this->db->get();
            $result = $query_result->result();

...
class Admin_Model extends MY_Model
{
    ...
    public function get_taxes_dropdown($name, $taxname, $type = '', $item_id = '', $is_edit = false, $manual = false)
    {
        ...
        $select = '<select class="select_box form-control display-block Rtax tax select2-multiple" name="' . $name . '"  data-placeholder="' . lang('no_tax') . '">';
        foreach ($taxes as $tax) {
            $selected = '';
            if (is_array($taxname)) {
    ...
    public function get_todo_status($status="")
    {
        $statuses = array(
            array(
                'id' => 1,
    ...
    * @desc return history of proposal url hits
    * @param request $request
    * @date 05 dec 21
    * @return array object
    */ 
    public function getProposalHistory()
    {
        $this->db->select('proposal_open_time,proposal_id,proposal_url,SEC_TO_TIME( SUM( TIME_TO_SEC( `duration` ) ) ) AS timeSum , COUNT(proposal_id) AS total_view', FALSE);
        $this->db->group_by("proposal_url");
        return $qwer = $this->db->get('tbl_proposal_history ')->result();
    }
}